...
public class BuildingManager : MonoBehaviour
{
    ...
    public void OnPlayerEnter()
    {
        ...
        {
            if (ScoreSignals.Instance.onGetScore(ScoreVariableType.TotalScore) > 0)
            {
                ScoreSignals.Instance.onChangeScore(ScoreTypes.DecScore,ScoreVariableType.TotalScore); //bina skor azalt 
        ...
        else if (_mainBuildingComplateState == BuildingComplateState.Completed && _sideBuildingComplateState == BuildingComplateState.Uncompleted)
        {
            if (ScoreSignals.Instance.onGetScore(ScoreVariableType.TotalScore) > 0)
            {
                ScoreSignals.Instance.onChangeScore(ScoreTypes.DecScore, ScoreVariableType.TotalScore);
                ScoreSignals.Instance.onUpdateScore?.Invoke();
               //PlayerSignals.Instance.onScaleDown?.Invoke();
                CheckComplateState(false, _sideBuildingComplateState, _sidePayedAmount, _sidePrice);
                _sidePayedAmount++;
